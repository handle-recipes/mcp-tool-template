...
export const createRecipeTools = (api: FirebaseFunctionsAPI) => [
  ...
  createMCPTool({
    ...
    schema: z.object({
      ...
      allergens: z
        .array(z.string())
        .optional()
        .describe("Known allergens in this ingredient"),
    }),
  ...
  createMCPTool({
    ...
    description: "Create a new suggestion for a recipe",
    schema: z.object({
  ...
  createMCPTool({
    ...
    schema: z.object({
      ...
      offset: z
        .number()
        .optional()
        .describe("Number of suggestions to skip"),
    }),
    handler: async ({ status, limit, offset }) => {
      ...
      const suggestionsList = result.suggestions
        .map(
          (sug) =>
            `- ${sug.title} (${sug.id}) - ${sug.category}, Priority: ${sug.priority}, Votes: ${sug.votes}, Status: ${sug.status}`
        )
        .join("\n");
  ...
  createMCPTool({
    ...
    description: "Vote for a suggestion (toggles vote)",
    schema: z.object({
